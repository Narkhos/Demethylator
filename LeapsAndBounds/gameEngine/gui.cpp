...
{
	drawImage(this->texId, x, y, w, h, 1.0f);
}

GLuint Texture::getId() { return this->texId;  }

// class TexturePool


TexturePool::TexturePool(std::string _path)
	:path(_path)
{
}
...
Texture* TexturePool::getTexture(std::string name)
{
	...
	it = textures.find(fullname);
	if (it == textures.end())
	{
		textures[fullname] = new Texture(fullname);
